...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h3 class="text-2xl font-bold text-slate-800">Nueva Transacción</h3>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <form method="POST" action="{{ route('transacciones.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-2 p-3 border rounded-lg cursor-pointer has-[:checked]:bg-emerald-50 has-[:checked]:border-emerald-300 has-[:checked]:ring-1 has-[:checked]:ring-emerald-300">
                        <input type="radio" name="tipo" value="ingreso" {{ old('tipo') == 'ingreso' ? 'checked' : '' }} class="text-emerald-600" required>
                    ...
                    <label class="flex items-center gap-2 p-3 border rounded-lg cursor-pointer has-[:checked]:bg-rose-50 has-[:checked]:border-rose-300 has-[:checked]:ring-1 has-[:checked]:ring-rose-300">
                        <input type="radio" name="tipo" value="gasto" {{ old('tipo') == 'gasto' ? 'checked' : '' }} class="text-rose-600" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="categoria_id" class="w-full px-3 py-2 border border-slate-200 rounded-lg" required>
                        ...
                        <option value="{{ $categoria->id }}" data-tipo="{{ $categoria->tipo }}" {{ old('categoria_id') == $categoria->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="monto" value="{{ old('monto') }}" step="0.01" min="0.01" placeholder="0.00" class="w-full pl-8 pr-3 py-2 border border-slate-200 rounded-lg" required>
                    </div>
                ...
                <div>
                    ...
                    <input type="date" name="fecha" value="{{ old('fecha', date('Y-m-d')) }}" class="w-full px-3 py-2 border border-slate-200 rounded-lg" required>
                ...
                <div>
                    ...
                    <select name="metodo_pago" class="w-full px-3 py-2 border border-slate-200 rounded-lg" required>
                        ...
                        <option value="electronico" {{ old('metodo_pago') == 'electronico' ? 'selected' : '' }}>Electrónico</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="descripcion" rows="3" placeholder="Descripción opcional de la transacción..." class="w-full px-3 py-2 border border-slate-200 rounded-lg">{{ old('descripcion') }}</textarea>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-slate-100">
                ...
                <a href="{{ route('transacciones.index') }}" class="px-6 py-2 border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors">
                    Cancelar
                </a>
            </div>
        </form>
