...
@section('title', 'Editar Asiento #' . $asiento->numero_asiento)
...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h3 class="text-2xl font-bold text-slate-800">Editar Asiento #{{ $asiento->numero_asiento }}</h3>
            <p class="text-slate-500">Modificar asiento contable</p>
        </div>
    </div>

    <form method="POST" action="{{ route('contabilidad.update', $asiento) }}" id="asientoForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" value="{{ $asiento->numero_asiento }}" disabled
                        class="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm bg-slate-50 text-slate-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Fecha</label>
                    <input type="date" name="fecha" value="{{ old('fecha', $asiento->fecha->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <textarea name="descripcion" rows="2"
                    class="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-slate-900 focus:border-slate-900 @error('descripcion') border-red-300 @enderror">{{ old('descripcion', $asiento->descripcion) }}</textarea>
                @error('descripcion')
                <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6 mt-6">
            ...
            <div id="partidasContainer" class="space-y-3">
                ...
                <div class="partida-row grid grid-cols-12 gap-3">
                    <div class="col-span-4">
                        <select name="partidas[{{ $loop->index }}][cuenta_id]"
                            ...
                            <option value="{{ $cuenta->id }}" {{ $partida->cuenta_id == $cuenta->id ? 'selected' : '' }}>
                    ...
                    <div class="col-span-2">
                        <input type="number" step="0.01" min="0" name="partidas[{{ $loop->index }}][debe]" value="{{ $partida->debe > 0 ? $partida->debe : '' }}"
                    ...
                    <div class="col-span-2">
                        <input type="number" step="0.01" min="0" name="partidas[{{ $loop->index }}][haber]" value="{{ $partida->haber > 0 ? $partida->haber : '' }}"
                    ...
                    <div class="col-span-3">
                        <input type="text" name="partidas[{{ $loop->index }}][descripcion]" value="{{ $partida->descripcion }}"
                            class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-slate-900 focus:border-slate-900"
        ...
        <div class="flex items-center justify-end gap-3 mt-6">
            <a href="{{ route('contabilidad.show', $asiento) }}" class="px-6 py-2.5 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">Cancelar</a>
            <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-slate-900 rounded-lg hover:bg-slate-800 transition-colors">Actualizar Asiento</button>
        </div>
    </form>
</div>
@endsection
